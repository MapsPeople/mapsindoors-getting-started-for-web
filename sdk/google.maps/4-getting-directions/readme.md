...
* Using `mapsindoors.mapView.GoogleMapsView` for integrating MapsIndoors with Google Maps.

* Using `mapsindoors.directions.GoogleMapsProvider` as the external directions provider for Google Maps.
...
// Get the underlying Google Maps map instance
...
// Add the floor selector HTML element to the Google Maps map using Google Maps' controls array
...
* **Route Calculation:**
  ...
  * When both origin and destination are set, clicking "Show Route" calls [`DirectionsService.getRoute()`](https://app.mapsindoors.com/mapsindoors/js/sdk/latest/docs/mapsindoors.services.DirectionsService.html#getRoute) to calculate a route between them. The `DirectionsService` is now initialized with `new mapsindoors.directions.GoogleMapsProvider()`.
